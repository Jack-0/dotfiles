...
# pretty format zsh per dir history
 ...
 grep -ri $1 $HOME/.directory_history --color=always \
     ...
     | awk '{cmd = "date -r \"$2\" +\"%Y-%m-%d %H:%M:%S\""; cmd | getline date_str; close(cmd); $2 = date_str; printf "\033[33m%s \033[37m%s", date_str, $1, "\n"; for (i = 3; i <= NF; i++) printf "\033[36m %s", $i; printf "\033[0m\n"}'
